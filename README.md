...
1. 函数名称`np.loadtxt(fname,dtype=np.float,delimiter=None,skiprows=0,usecols=None,unpack=False)`
    1. fname,文件/字符串或产生器,可以是.gz或bz2压缩文件
    2. dtype,数据类型,默认是np.float
    3. delimiter,分隔字符串,默认是任何空格
    4. skiprows,跳过前n行
    5. usecols,读取指定的列,索引,元组类型
    6. unpack,如果为True,读入属性将分别写入不同数组变量,Flase读入数据只写入一个数组变量,默认为False
